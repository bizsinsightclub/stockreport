...
### 3. 슬롯 6개 작성 (한국어, 사실 기반, **매집/매도 시그널 포함**)

| Slot | 분량 | 내용 |
|---|---|---|
| `brief` | 3 문장 | 가격·등락률·거래량 + 핵심 수급 + 가격 위치/추세 한 줄 요약 |
| `s1_price` | 3 문장 | 단일일 외/기/개 순매수 + 5거래일 누적 + 매집 관점 트리거 |
| `s2_tech` | 4 문장 | RSI/MA/MACD/Bollinger 시그널 + **매집 관점 진입 영역** + **매도 관점 차익실현 영역** |
| `s3_disc_0`, `s3_disc_1`, … | 1 문장 | 각 공시 행 한 줄 평 (공시 개수만큼 작성, 인덱스는 `disclosures` 배열 순서) |
| `s4_peer` | 3–4 문장 | 피어별 등락률 비교 + 업종 동행성/차별화 + 단기 진입 강도 시사점 |
| `s4_macro` | 2 문장 | USD/KRW 수치·델타 + 종목 업종 대비 환율 영향 한 줄 평 |

**매집/매도 시그널 작성 가이드** (s2_tech 핵심):
- 매집 관점: RSI 진입 영역 (예: 30→50 회복, 50 회귀), 외국인/기관 매도 진정 시점, 골든 구조 + MACD 상승 동시 발생 등 진입 검토 트리거를 시그널로 명시
- 매도/차익실현 관점: RSI 70 부근 과열, 신고가 갱신 실패, 골든 구조 훼손 등 차익실현 검토 트리거 시그널 명시
- 명시적 매수/매도 추천(투자 조언) 어조는 피하고 "분할 진입 검토 가능 구간", "차익실현 검토 가능" 같은 분석 어조 사용

원칙:
- 정성적 표현 (예: "강세 추세 유지", "박스권 하단 재테스트") 은 자유, **모든 수치는 raw 출처에 정확히 일치해야 한다**.
